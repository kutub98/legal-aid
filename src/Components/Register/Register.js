...
const Register = () => {
  ...
  const { auth, createWithEp } = useContext(AuthContext);
  ...
  const registeFormHandler = (e) => {
    ...
    console.log("name", name + "photoUrl", PhotoUrl, "email", email, "password", password, "confirmPass", ConfirmPass);

    if (password.length < 6) {
      setError("Password must have at least six characters");
    }
    if (password !== ConfirmPass) {
      setError("Password did not matching");
      return
    }

    createWithEp(email, password)
      .then((res) => {
        const user = res.user;
        Swal.fire({
            position: 'center',
    ...
    <div className="loginBox">
      ...
      <div className="loginForm w-1/2">
        <div className="w-full max-w-md p-8 space-y-3 rounded-xl bg-gray-200 text-gray-800">
          ...
          <form onSubmit={registeFormHandler} className="space-y-6 ng-untouched ng-pristine ng-valid">
            ...
            <button type="submit" className="block w-full p-3 text-center rounded-sm text-gray-50 bg-yellow-800 hover:bg-slate-900">
              Create Account
            </button>
          </form>
