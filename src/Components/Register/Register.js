...
const Register = () => {
  ...
  const registeFormHandler = e => {
    ...
    createWithEp(email, password).then(res => {
      const user = res.user;
      Swal.fire({
        position: 'center',
        icon: 'success',
      ...
      <div className="loginForm lg:w-1/2 w-full">
        <div className="w-full lg:max-w-md w-full p-8 space-y-3 rounded-xl bg-gray-200 text-gray-800">
          ...
          >
            ...
            <button
              type="submit"
              className="block w-full p-3 text-center rounded-sm text-gray-50 bg-yellow-800 hover:bg-slate-900"
            >
              Create Account
            </button>
          </form>
